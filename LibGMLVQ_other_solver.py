...
from minimize_wrapper import cstm_minimize


class LibGMLVQ_other_solver(GlvqModel):
    ...
    def __init__(self, prototypes_per_class=1, initial_prototypes=None,
                 ...
                 random_state=None, alg="scipy", matlab_options=None, nb_reiterations=1):
        super(LibGMLVQ_other_solver, self).__init__(prototypes_per_class,
    ...
    def _optimize(self, x, y, random_state):
        ...
        if self.initial_matrix is None:
            if self.dim_ == nb_features:
                ...
                # self.omega_ = np.diag(np.abs(np.mean(self.w_, axis=0) / np.sum(np.mean(self.w_, axis=0))))
            else:
        ...
        self.omega_ /= np.sqrt(np.trace(self.omega_ @ self.omega_))  # TODO hinzugefügt, in matlab auch so

        variables = np.append(self.w_, self.omega_, axis=0)
        label_equals_prototype = y[np.newaxis].T == self.c_w_
        method = 'l-bfgs-b'
        res_x, nit, _ = cstm_minimize(
            fun=lambda vs:
            ...
            alg=self.alg,
            matlab_options=self.matlab_options.copy(),
            options={'disp': self.display, 'gtol': self.gtol,
                     'maxiter': self.max_iter})
        n_iter = nit
        res_x, nit, fval = cstm_minimize(
            fun=lambda vs:
            self._optfun(vs, x, label_equals_prototype=label_equals_prototype),
            jac=lambda vs:
            self._optgrad(vs, x, label_equals_prototype=label_equals_prototype,
            ...
            method=method, x0=res_x,
            alg=self.alg,
            matlab_options=self.matlab_options.copy(),
            options={'disp': self.display, 'gtol': self.gtol,
                     'maxiter': self.max_iter})
        n_iter = max(n_iter, nit)
        for reiter in range(self.nb_reiterations):
            res_x, nit, new_fval = cstm_minimize(
                fun=lambda vs:
                self._optfun(vs, x, label_equals_prototype=label_equals_prototype),
                jac=lambda vs:
                self._optgrad(vs, x, label_equals_prototype=label_equals_prototype,
            ...
            if not new_fval:
                break
            if np.abs(new_fval - fval) < 1e-3:
                break
            fval = new_fval
        out = res_x.reshape(res_x.size // nb_features, nb_features)
