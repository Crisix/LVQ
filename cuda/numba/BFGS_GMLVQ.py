...
class MyBFGSCudaGMLVQ(BaseEstimator, ClassifierMixin):
    """
...
@cuda.jit(f"void(float64[:, :], float64[:, :], int64[:], float64, float64, float64[:, :], float64[:, :], int64[:], float64[:, :], float64[:, :], float64[:])")
        ...
        if lr_omega > 0:
            ...
            for l in range(omega.shape[0]):
                for m in range(omega.shape[1]):
                    ...
                    # noinspection PyArgumentList
                    # das ist meins, 1:1 aus dem GMLVQ Paper!
                    # cuda.atomic.add(omega_grad, (l, m), -lr_omega * dfdmu * (mu_plus * fst - mu_minus * snd))

                    # noinspection PyArgumentList
                    # das ist nicht meins, 1:1 aus sklearn-lvq
                    cuda.atomic.add(omega_grad, (l, m), -lr_omega * dfdmu * (mu_plus * fst - mu_minus * snd))
