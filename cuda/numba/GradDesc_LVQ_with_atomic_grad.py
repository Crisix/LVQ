...
class MyCudaGradDescGMLVQ(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y):
        ...
        for e in range(self.iterations):
            ...
            dev_omega_grad = cuda.to_device(np.zeros_like(self.w_))
            dev_w_grad = cuda.to_device(np.zeros_like(self.w_))

            cuda_fit_iteration[grid_dim, block_dim](dev_w, dev_X, dev_y, self.lr, self.matrix_lr, self.omega, self.omega @ self.omega, self.classes_, dev_omega_grad, dev_w_grad)

            w_grad = dev_w_grad.copy_to_host()
            # self.w_ = dev_w.copy_to_host()
            # self.w_ += (w_grad / X.shape[0])
            self.w_ += w_grad
...
def cuda_fit_iteration(w: np.ndarray, X: np.ndarray, y: np.ndarray, lr: float, matrix_lr: float, omega: np.ndarray, Lambda: np.ndarray, classes: np.ndarray,
                       omega_change: np.ndarray, w_grad):
    i = cuda.grid(1)
...
for ITER in [2000]:
    ...
    for _ in range(5):
        ...
        [test_performance(m, plot=False) for m in models]
        print("")
